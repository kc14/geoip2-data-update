...
"${BEELINE_CMD}" --fastConnect=true -u "${BEELINE_JDBC_URL}" -n pentaho --verbose -e "DESCRIBE FORMATTED maxmind_geoip2_staging.geoip2_city_ipv6_and_4 PARTITION (pdate = '${PDATE_SOURCE}');" \
| grep -e '^| Location:' | cut -d '|' -f 3 | python -c "import sys; print sys.stdin.readline().strip()"
)

"${BEELINE_CMD}" --fastConnect=true -u "${BEELINE_JDBC_URL}" -n pentaho --verbose -e "ALTER TABLE maxmind_geoip2_staging.geoip2_city_ipv6_and_4 ADD PARTITION (pdate = '${PDATE_TARGET}') LOCATION '${location_pdate_source}';"
